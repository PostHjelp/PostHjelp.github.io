document.getElementById('nav-innleggstavle').addEventListener('click', () => {
    window.location.href = '/index.html';
});

document.getElementById('nav-arbeidsportal').addEventListener('click', () => {
    window.location.href = '/html/arbeidsportal.html';
});

document.getElementById('nav-vikarportal').addEventListener('click', () => {
    window.location.href = '/html/vikarportal.html';
});

document.getElementById('my-profile-logo').addEventListener('click', () => {
    window.location.href = '/html/min_side.html'
});